		...
		extern int game_parse_info(t_game * game, char const * gameinfo)
		{
			...
			*/
			...
			else if (clienttag == CLIENTTAG_WARCRAFT3_UINT || clienttag == CLIENTTAG_WAR3XP_UINT)
			{
				...
				// W3 StatString format (after decode):
				// [5 bytes settings]
				// [1 byte 0x00]
				// [2 bytes mapWidth]
				// [2 bytes mapHeight]
				// [4 bytes mapCRC]
				// [string mapname + 0x00]
				// [string hostName + 0x00]
				// [20 bytes SHA1]
				// [string ownerHostName + 0x00] [CUSTOM]
				// [1 byte maxPlayers] [CUSTOM]
				// [0x00 end]

				// Parse mapname (offset 13)
				offset = 13;
				const char* mapname = pstr + offset;
				game_set_mapname(game, mapname);
				eventlog(eventlog_level_info, __FUNCTION__, "parsed mapname: '{}'", mapname);
				...
				if (hostName[0] != '\0')
				{
					...
					// Parse ownerHostName
					const char* ownerHostName = pstr + offset;
					if (ownerHostName[0] != '\0')
					{
						game_set_owner_name(game, ownerHostName);
						eventlog(eventlog_level_info, __FUNCTION__, "parsed ownerHostName: '{}'", ownerHostName);
						offset += std::strlen(ownerHostName) + 1;

						unsigned char max_players = *(unsigned char*)(pstr + offset);
						game_set_maxplayersmap(game, max_players);
					}
					else
					{
						game_set_owner_name(game, "?");
						game_set_maxplayersmap(game, 0);
					}
				}
				else
				{
					game_set_hostName(game, "?");
					game_set_owner_name(game, "?");
					game_set_maxplayersmap(game, 0);
				}
