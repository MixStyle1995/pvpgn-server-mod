...
namespace pvpgn
{
	...
	namespace bnetd
	{
		...
		extern t_game* game_create(char const* name, char const* pass, char const* info, t_game_type type, int startver, t_clienttag clienttag, unsigned long gameversion)
		{
			...
			//mod by thai son
			game->last_host_info_broadcast = 0;
			game->maxplayersmap = 0;
			game->hostName = NULL;
			game->owner_name = NULL;
		...
		extern int game_set_hostName(t_game* game, char const* hostName)
		{
			...
			if (!hostName)
			{
				eventlog(eventlog_level_error, __FUNCTION__, "got NULL hostName");
				return -1;
			}

			if (game->hostName != NULL) xfree((void*)game->hostName);

			game->hostName = xstrdup(hostName);

			return 0;
		}

		extern char const* game_get_hostName(t_game const* game)
		{
			if (!game)
			{
				eventlog(eventlog_level_error, __FUNCTION__, "got NULL game");
		...
		extern int game_set_owner_name(t_game* game, char const* owner_name)
		{
			if (!game)
			{
				eventlog(eventlog_level_error, __FUNCTION__, "got NULL game");
			...
			if (!owner_name)
			{
				eventlog(eventlog_level_error, __FUNCTION__, "got NULL owner_name");
				return -1;
			}

			if (game->owner_name != NULL) xfree((void*)game->owner_name);

			game->owner_name = xstrdup(owner_name);

			return 0;
		}

		extern char const* game_get_owner_name(t_game const* game)
		{
			if (!game)
			{
				eventlog(eventlog_level_error, __FUNCTION__, "got NULL game");
			...
			return game->owner_name;
		}

		extern unsigned int game_get_maxplayersmap(t_game const* game)
		{
			if (!game)
			{
				eventlog(eventlog_level_error, __FUNCTION__, "got NULL game");
			...
			return game->maxplayersmap;
		}

		extern int game_set_maxplayersmap(t_game* game, unsigned int maxplayers)
		{
			if (!game)
			{
				eventlog(eventlog_level_error, __FUNCTION__, "got NULL game");
		...
		extern int game_send_list_to_connection(t_connection* c)
		{
			...
			elist_for_each(pos, gamelist())
			{
				...
				if (game)
				{
					...
					bn_byte_set(&game_data.slots_total, game_get_maxplayers(game));
					{
						char const* info = game_get_info(game);
						size_t infoLen = info ? std::strlen(info) : 0;
						if (infoLen >= sizeof(game_data.stat_string))
							infoLen = sizeof(game_data.stat_string) - 1;
						if (info) std::memcpy(game_data.stat_string, info, infoLen);
						bn_short_set(&game_data.stat_string_len, (unsigned short)infoLen);
					}
